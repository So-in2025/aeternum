...
try:
    ...
    # Intentamos conectar con las variables que cargaste en Vercel
...
# Datos por si la DB no está disponible
NODOS_DB_FALLBACK = {
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # 2. HANDSHAKE PARA EL EXE (shadow_brain.py)
        ...
        for nid, d in nodes.items():
            ...
            <tr class="border-b border-zinc-800 hover:bg-zinc-800/50 transition">
                ...
                <td class="p-4 flex gap-2">
                    <a href="/api/action?id={nid}&status=OK" class="bg-green-900/30 text-green-400 px-2 py-1 rounded text-[10px] border border-green-800 font-bold hover:bg-green-500 hover:text-white transition">ACTIVAR</a>
                    <a href="/api/action?id={nid}&status=PURGE" class="bg-red-900/30 text-red-400 px-2 py-1 rounded text-[10px] border border-red-800 font-bold hover:bg-red-500 hover:text-white transition">PURGAR</a>
                </td>
            </tr>"""

        db_status = "ONLINE" if HAS_DB else "OFFLINE"
        db_color = "text-green-500" if HAS_DB else "text-red-500"
        ...
        <body class="p-6 md:p-10">
            <div class="max-w-5xl mx-auto">
                <header class="flex justify-between items-center mb-10 border-b border-zinc-800 pb-5">
                    ...
                    <div class="text-right text-[10px] uppercase">
                        <p>SISTEMA: <span class="text-white">ACTIVO</span></p>
                        <p>DB: <span class="{db_color}">{db_status}</span></p>
                    </div>
                </header>
                <div class="matrix-border bg-zinc-900/50 rounded-xl overflow-hidden">
                    <table class="w-full text-left">
                        <thead class="bg-zinc-800/30 text-zinc-400 text-xs uppercase">
                            <tr>
